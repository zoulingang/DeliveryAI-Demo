...
export function CartPanel({ items, compact, onQuantity, onSubmit }: CartPanelProps) {
  ...
  if (!items.length) {
    return (
      <section className="rounded-3xl border border-charcoal-900/5 bg-white p-6 text-center shadow-card dark:border-white/5 dark:bg-charcoal-700">
  ...
  return (
    <section className={`rounded-3xl border border-charcoal-900/5 bg-white shadow-card dark:border-white/5 dark:bg-charcoal-700 ${compact ? 'p-4' : 'p-5'}`}>
      <div className="flex items-center justify-between">
        <div><h3 className="text-lg font-extrabold text-charcoal-900 dark:text-rice-100">{t('cart.title')}</h3><p className="mt-1 text-xs text-charcoal-500 dark:text-rice-200/60">{t('cart.item_count', { count: items.reduce((sum, item) => sum + item.quantity, 0) })}</p></div>
        <span className="flex items-center gap-1 rounded-full bg-amber-100 px-3 py-1 text-xs font-bold text-charcoal-700 dark:bg-amber-500/15 dark:text-amber-400"><Users size={13} />{t('cart.people', { count: people.length })}</span>
      </div>
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-2 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2 rounded-lg bg-rice-100 p-1 dark:bg-charcoal-900">
                  <button onClick={() => onQuantity(item.uid, -1)} className="rounded-md bg-white p-1 text-charcoal-700 shadow-sm dark:bg-charcoal-700 dark:text-rice-200" aria-label={t('common.aria_reduce')}>{item.quantity === 1 ? <Trash2 size={13} /> : <Minus size={13} />}</button>
